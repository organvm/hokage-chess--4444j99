...
| 2026-05-12 | Cross-bleed hook fired on 3 authored operator-layer artifacts (`docs/business/design-docs/prompt-atom-registry.md`, `docs/business/forms/internal/x-1-session-re-entry.md`, `docs/business/forms/internal/x-2-user-actionable-deferrals.md`) — incidental Maddie/elevatealign references in hokage-internal docs whose subject IS operator coordination across streams | Whitelist extended with `docs/business/design-docs/.*` and `docs/business/forms/internal/.*` — structurally identical to already-whitelisted `HANDOFF.md` and `.conductor/active-handoff.md` (operator-layer artifacts that naturally accumulate cross-stream references as part of their own subject matter; *not* chat-layer ephemera that R1 routes off-repo) | `scripts/check-cross-client-bleed.sh` WHITELIST_REGEX extended |

### Whitelist Extension Rationale (2026-05-12)

The substrate's whitelist pattern carries an implicit principle: **artifacts whose subject is meta-organizational coordination, or which naturally accumulate cross-stream references because they catalog the operator's reality, are pre-whitelisted.** Already-included categories — substrate doc itself, canonical home anchor, the guard script, `HANDOFF.md`, `.conductor/active-handoff.md`, `docs/archive/*`, `.claude/plans/*` — all share this shape. They are *not* the content that R1 routes off-repo (chat-layer transcripts, accidental cross-stream drift). They are *deliberate* operator instruments.

The 2026-05-12 extension adds two paths the pattern always implicitly included but had not yet pattern-named:

- `docs/business/design-docs/.*` — hokage-internal design docs (registry, audit, ontology) where the *subject* is hokage's own data model and cross-stream signals appear because the data model catalogs all operator directives.
- `docs/business/forms/internal/.*` — hokage-internal operator forms (session re-entry checklists, deferral trackers) where cross-stream fields exist *because the operator manages multiple streams from a single coordination surface*.

This is governance *completion* — finishing a pattern the substrate was already partway through expressing. It does not loosen R1 for chat-layer artifacts (those still route off-repo per existing rules). It does not extend to client-facing surfaces (pitch decks, public docs, `docs/business/forms/rob/*` — all remain hook-checked because they are NOT operator-layer instruments).
