...
- `docs/governance/` — governance forwarding stubs and anchors. `client-separation-substrate.md` now points to the canonical workspace-wide copy at `meta-organvm/governance/client-separation-substrate.md`.
...
Client information must never bleed across clients or into public surfaces. The four zones (`public:all`, `(me)`, `{client:rob}`, `{client:maddie}`) and their flow rules are documented in the canonical workspace copy at `meta-organvm/governance/client-separation-substrate.md`; this repo keeps a forwarding stub at `docs/governance/client-separation-substrate.md`. Read the canonical doc before triaging any cross-scope artifact, before pasting external session content into a chat bound to this repo, or before writing files that name another client. Routing rule: route by content / scope-of-generation (R1), not by user-declared intent. Live-paste rule: if cross-client material lands in this scope's chat, refuse to operationalize, suggest correct routing, never absorb the wrong-scope content into auto-memory.
